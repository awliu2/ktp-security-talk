...
def main():
    ...
    while True:
        ...
        if parsed[0] == "help" or parsed[0] == "h":
            ...
            print(
                "* transfer <src> <dst> <amount> - transfer the given amount from the src card to the dst card"
            )
            ...
            if len(parsed) == 2:
                ...
                if manager.get_balance(card_id):
                    print(
                        f"Balance of card {card_id} is {manager.get_balance(card_id)}"
                    )
